...
def make_link(G, node1, node2, w, no, dur, start, end):
    ...
    if node2 not in G[node1]:
        (G[node1])[node2] = []
    if len((G[node1])[node2]) > 0:
        i = (G[node1])[node2][0]
        if w > i[0]:
            None
        elif w < i[0]:
            (G[node1])[node2] = [[w,no, dur, start,end]]
        else:
...
def dijkstra(G,v):
    ...
    while len(final_dist) < len(G):
        ...
        final_time[w] = time_so_far[w]
        del dist_so_far[w]   
        for x in G[w]:
            if x not in final_dist:
                if x not in dist_so_far:
                    dist_so_far[x] = final_dist[w] +  G[w][x] [0][0]
                    path_so_far[x] = final_path[w] + [x]
                    heapq.heappush(heap,(dist_so_far[x],x))
                elif final_dist[w] + G[w][x][0][0] < dist_so_far[x]:
...
def test2():
    G ={}
    G =pre_process(all_flights, G)
    print G
    for i in G:
        print i, G[i]
    a,b = dijkstra(G,'Mt Magnet')
    print a
    print b
