...
def bowling(balls):
    ...
    for i in range(len(balls)):
        if balls[i] == 10 and i < (len(balls)-2) and new_round[i]:
            ball_points.append((balls[i], balls[i+1], balls[i+2]))
            new_round[i+1] = True
        elif  balls[i] < 10  and i < (len(balls)-2) and new_round[i]:
            if balls[i] + balls[i+1] == 10:
                ball_points.append((balls[i], balls[i+1], balls[i+2]))
            else:
                print balls[i], balls[i+1]
                ball_points.append((balls[i],))
                ball_points.append((balls[i+1],))
            new_round[i+2] = True
        elif new_round[i]:
                ball_points.append((balls[i],))
                ball_points.append((balls[i+1],))
