...
def find_best_flights(flights, origin, destination):
    G = {}
    G=pre_process(flights, G)
    best_flights = search(G, origin, destination,0,0,flights) 
    return None if len(best_flights) ==0 else best_flights 
    # your code here
...
def make_link(G, node1, node2, w, no, dur, start, end):
    if node1 not in G:
        G[node1] = {}
    if node2 not in G[node1]:
        (G[node1])[node2] = []
    (G[node1])[node2].append( [w,no, dur, start,end])

    return G
def pre_process(F, G):
    for i in F:
        make_link(G, i[1],i[2],i[5],i[0],calc_time(i[3],i[4]),i[3],i[4])
    return G
def calc_time(a, b):
    min_a=a.split(':')[1]
    hour_a=a.split(':')[0]
    min_b=b.split(':')[1]
    hour_b=b.split(':')[0]  
    return int((60-int(min_a)) + int(min_b) + (int(hour_b) - int(hour_a) -1)*60)
...
def is_greater(cur_dest_h,cur_dest_m,new_h, new_m ):
    if new_h > cur_dest_h:
        return True
    elif new_h == cur_dest_h and new_m >= cur_dest_m:
        return True
    else:
        return False
...
def shortest_dist_node(dist):
    ...
    best_value = 1000000
    for v in dist:
        if dist[v][0] < best_value:
            (best_node, best_value) = (v, dist[v])
    return best_node


def search(G,v, u, start, end, flights):
    paths = []
    for node in G[v]:
        for flight in G[v][node]:
            flight_start_hour = flight[3].split(':')[0]
            flight_start_min = flight[3].split(':')[1]
            flight_end_hour = flight[4].split(':')[0]
            flight_end_min = flight[4].split(':')[1]
            flight_no = flight[1]
            if is_greater(start, end,  flight_start_hour, flight_start_min):
                if node == u:
                    paths.append( [flight_no])
                else:
                    paths.append( [flight_no] +  search(G, node, u, flight_end_hour, flight_end_min,  flights))

    ef = ending_flights(all_flights,u)
    final_paths = remove_path(paths,ef)
    ...
    if len(final_paths) == 0:
        ...
        return best(final_paths, all_flights)

def remove_path(paths,ef):
    final_paths = []
    for i in paths:
        if i[-1]  in ef:
...
def ending_flights(all_flights,u):
    ef = []
...
def best(paths, flights):
    best_path = paths[0]
    for path in paths:
        if cost(path, flights) < cost(best_path, flights):
            best_path = path
        elif cost(path, flights)  == cost(best_path, flights):
            if running_time(path, flights) < running_time(best_path, flights):
                best_path = path
    return best_path

def cost(path, flights):
    sum1 = 0
    for i in flights:
        if i[0] in path:
            sum1 += i[5]
...
def running_time(path, flights):
    for i in flights:
        if i[0] == path[0]:
            f = i
        if i[0] == path[-1]:
            l = i
    return convert_min(l[4]) - convert_min(f[3])
